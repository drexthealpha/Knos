...
| | measured |
|---|---|
| the compare-and-swap that refuses a claim | 0.24 ms |
| one MCP tool call, which opens the store and closes it | 3.7 ms |
| a cold `knos` command, the slowest path here | 11 ms |
| one real source file of this repo, read and parsed | 2.2 ms |
<!-- /measured -->

So a tool call pays for itself once it prevents **1.7** files of
duplicated work, and a cold command at **5.1**. Below that,
