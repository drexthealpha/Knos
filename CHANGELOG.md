...
`knos demo` gained a cold-start beat. A separate interpreter, handed nothing
but the repo path, prints its own pid alongside the repo's commit hash and the
wall clock and then reads back what an earlier process wrote. Recall across a
process boundary was always true and was never on screen; now the recall and
the deletion are the same unbroken minute.

`scripts/collide.py` measures the coordination claim instead of asserting it.
Sixteen operating-system processes reach for one topic at the same instant,
eight rounds: 0 double-grants in 128 attempts, and every refusal names the
agent actually holding it. The ablated condition is sharing rather than the
file - deleting the store proves nothing here, because the next agent recreates
it and the lock works again - so the second arm gives every agent its own
memory, which is what an agent has today. All sixteen then take the same work.
Numbers in `docs/evidence/collide.json`, regenerated by `tests/test_collide.py`.
